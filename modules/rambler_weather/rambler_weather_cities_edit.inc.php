...
*/
  ...
  $table_name='rambler_weather_cities';
  ...
  if ($this->tab=='data') {
   ...
   if ($this->mode=='update') {
    global $title_new;
	if ($title_new) {
	 $prop=array('TITLE'=>$title_new,'ID'=>$rec['ID']);
	 $new_id=SQLInsert('rambler_weather_values',$prop);
	}
   }
   global $delete_id;
   if ($delete_id) {
    SQLExec("DELETE FROM rambler_weather_values WHERE ID='".(int)$delete_id."'");
   }
   $properties=SQLSelect("SELECT * FROM rambler_weather_values WHERE CITY_ID='".$rec['ID']."' ORDER BY ID");
   ...
   for($i=0;$i<$total;$i++) {
    ...
    if ($this->mode=='update') {
      ...
      SQLUpdate('rambler_weather_values', $properties[$i]);
